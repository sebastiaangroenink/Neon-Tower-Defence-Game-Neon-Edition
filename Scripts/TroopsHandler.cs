...
public class TroopsHandler : MonoBehaviour {
    ...
    void Update() {
        if(health == 0 || health < 0) {
            Destroy(gameObject);
            //Remove from enemy list if not yet out of bounds
        }
    ...
    public void OnTriggerEnter(Collider other) {
        other.GetComponent<TowerHandler>().targets = target.Add(gameObject);// Moet hier ervoor zorgen dat als het object in raidus komt dat hij zichzelf add in de tower zijn target lijst. En removed als ie dood is of out of bounds.
    }
}
